...
ITERATIONS = 5000
CHARACTERS = ascii_letters + digits + punctuation # TODO(1): fill in the missing characters
BRUTE_FORCE_LIMIT = 3 # chars to brute force at the end
CHECK_PASS_LEN_VULN = check_password1 # TODO(2): fill in the correct function
CHECK_PASS_CHARS_VULN = check_password2 # TODO(2): fill in the correct function
...
def find_length() -> int:
    ...
    times = [
        ...
        for length in range(20) # TODO(3): fill in an appropriate range
    ]
    ...
    while not q.empty():
        ...
        if len(p) != length - BRUTE_FORCE_LIMIT:
            ...
            for c in CHARACTERS:
                ...
                duration = timed_check_pass(CHECK_PASS_CHARS_VULN, pad(np, length))# TODO(5): do a timed password check
                q.put((-duration, np))
        else:
